...
//        if (currentUser != null && currentUser.getPwd().equals(post.getUser().getPwd())) {
...
//        if (post == null || !post.getUser().getPwd().equals(user.getPwd())) {
...
//
//        User user = (User) session.getAttribute("user");
//        if (user == null || existingPost == null || !existingPost.getUser().getPwd().equals(user.getPwd())) {
...
//        }
//
//        List<Comment> comments = commentRepository.findByUserId(loggedInUser.getId());
//        model.addAttribute("comments", comments);
//
